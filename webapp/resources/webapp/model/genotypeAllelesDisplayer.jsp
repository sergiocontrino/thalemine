...
<c:set var="alleles" value="${object.alleles}" />

<jsp:useBean id="formatter"
	class="org.thalemine.web.formatter.AlleleFormatter" />
...
<div id="genotype_allele_displayer" class="collection-table">
	<c:set var="rowCount" value="${fn:length(alleles)}" />
		...
		<c:when test="${!empty alleles}">
			...
			<table>
				...
				<tbody>
					...
					<c:forEach var="item" items="${alleles}">
						<tr>
						<c:set var="name" value="${item.primaryIdentifier}" />
						
						<c:set var="formattedName"
							value="${formatter.getFormattedName(name)}" />

						<td><a href="report.do?id=${item.id}"><i>${formattedName}</i></a></td>
						
						<c:set var="sequenceAlterationName" value="${item.sequenceAlterationType.name}" />
						<c:set var="type" value="sequenceAlterationType" />
						
						<c:set var="sequenceAlteration"
							value="${formatter.getFormattedAttributeName(type, sequenceAlterationName)}" />
							
						<td>${sequenceAlteration}</td>
						
						<c:set var="mutagenName" value="${item.mutagen.name}" />
						<c:set var="type" value="mutagen" />
						
						<c:set var="mutagen"
							value="${formatter.getFormattedAttributeName(type, mutagenName)}" />
							
												
						<td>${mutagen}</td>
						
						<c:set var="alleleClassName" value="${item.alleleClass.name}" />
						<c:set var="type" value="alleleClass" />
						
						<c:set var="alleleClass"
							value="${formatter.getFormattedAttributeName(type, alleleClassName)}" />
							
						<td>${alleleClass}</td>
						
						<c:set var="inheritanceTypeName" value="${item.inheritanceMode.name}" />
						<c:set var="type" value="inheritanceType" />
						
						<c:set var="inheritanceType"
							value="${formatter.getFormattedAttributeName(type, inheritanceTypeName)}" />
