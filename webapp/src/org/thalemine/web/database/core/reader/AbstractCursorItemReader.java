package org.thalemine.web.database.core.reader;
...
import org.thalemine.web.database.core.ReaderNotOpenException;
import org.thalemine.web.database.reader.util.JdbcUtils;
...
public abstract class AbstractCursorItemReader<T> extends AbstractItemCountingItemStreamItemReader<T> {
	...
	protected T doRead() throws Exception {
		...
		try {
			...
			log.info("Current Row Count 1:" +currentItemCount);
			
			if (!rs.next()) {
				
				log.info("No Records in ResultSet!");
				
				return null;
				}else{
				log.info("Found Records in ResultSet!");
			}
			...
			log.info("Current Row Count 2:" +currentItemCount);
